...
# Initialize session state with shuffled quiz data
...
# Function to reset quiz with new shuffle
def reset_quiz():
    st.session_state.quiz_order = list(range(len(quiz_data)))
    random.shuffle(st.session_state.quiz_order)
    st.session_state.current_index = 0
...
if role == "Student":
    if not st.session_state.user_details:
        ...
        # Quiz logic
        if not st.session_state.time_up and st.session_state.current_index < len(quiz_data):
            question_index = st.session_state.quiz_order[st.session_state.current_index]
